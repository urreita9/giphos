...
function suggestions(input) {
	return fetch(
		'https://api.giphy.com/v1/tags/related/' +
			input +
			'?&api_key=940D6rFjWipAwUmqdskYmMK8wMV036ID&limit=4'
	)
...
function search(limit, offset) {
	...
	let apiSearch =
		`https://api.giphy.com/v1/gifs/search?${apiKey}${searchValue}&offset=${offset}&limit=` +
		limit;
...
function addTrashIcon(likeBtn) {
	favsTitle.textContent === 'Favoritos'
		? likeBtn.classList.add('on-favs')
		: likeBtn.classList.remove('on-favs');
	favsTitle.textContent === 'Mis GIFOS'
		? likeBtn.classList.add('on-favs')
		: likeBtn.classList.remove('on-favs');
}
...
function drawHover(giphoDivContainer, selectedGif) {
	...
	likeBtn.addEventListener('click', () => {
		if (
			selectedGif.number == 1 ||
			selectedGif.number == 2 ||
			selectedGif.number == 3
		) {
...
function nextOn(fullscreenContainer, selectedGif) {
	...
	if (selectedGif.number == 1) {
		...
		trendingOffset++;
		trending(1, trendingOffset).then((data) =>
			fullScreen({
				url: data[0].images.original.url,
				title: data[0].title,
				username: data[0].username,
...
function nextOnFavorites(selectedGif) {
	...
	favs == favoritos.length - 1
		? fullScreen(favoritos[0])
		: fullScreen(favoritos[favs + 1]);
}
function nextOnFullscreen(selectedGif, sectionArray) {
	let fullGifs = sectionArray.findIndex((gif) => gif.id === selectedGif.id);
	fullGifs == sectionArray.length - 1
		? fullScreen(sectionArray[0])
		: fullScreen(sectionArray[fullGifs + 1]);
}
function previousOnFullscreen(selectedGif, sectionArray) {
	let fullGifs = sectionArray.findIndex((gif) => gif.id === selectedGif.id);
	fullGifs == 0
		? fullScreen(sectionArray[sectionArray.length - 1])
		: fullScreen(sectionArray[fullGifs - 1]);
}
...
function previousOnFavorites(selectedGif) {
	...
	let favs = favoritos.findIndex((fav) => fav.id === selectedGif.id);
	favs == 0
		? fullScreen(favoritos[favoritos.length - 1])
		: fullScreen(favoritos[favs - 1]);
}
function nextOnMisGifos(selectedGif) {
	...
	gifos == misGifos.length - 1
		? fullScreen(misGifos[0])
		: fullScreen(misGifos[gifos + 1]);
}
function previousOnMisGifos(selectedGif) {
	...
	let gifos = misGifos.findIndex((gif) => gif.id === selectedGif.id);
	gifos == 0
		? fullScreen(misGifos[misGifos.length - 1])
		: fullScreen(misGifos[gifos - 1]);
}
...
function trending(trendingLimit, trendingOffset) {
	return (
		fetch(
			`https://api.giphy.com/v1/gifs/trending?api_key=940D6rFjWipAwUmqdskYmMK8wMV036ID&q=&limit=` +
				trendingLimit +
				`&offset=` +
				trendingOffset
		)
...
function trendingTopics() {
	return fetch(
		'https://api.giphy.com/v1/trending/searches?api_key=940D6rFjWipAwUmqdskYmMK8wMV036ID'
	)
		.then((response) => response.json())
		.then((data) => data)
		.catch((error) => console.error(error));
...
function drawTrendingTopics(dataApiObject) {
	...
	for (let i = 0; i < 5; i++) {
		let myTitles =
			trendingArray[i].charAt(0).toUpperCase() + trendingArray[i].slice(1);
...
function drawTrendsOnLoad(data) {
	let giphosContainer = document.getElementById(
		'trending-scroll-image-container'
	);
...
function getFavorites() {
	...
	if (!favoritos || favoritos.length === 0) {
		...
		for (
			...
			i < 12 * currentPage && i < favoritos.length;
			i++
		) {
			let giphoDivContainer = document.createElement('div');
			giphoDivContainer.classList.add('gipho-div-container');
...
function uploadMyGifo(form) {
	return fetch(
		`https://upload.giphy.com/v1/gifs?api_key=940D6rFjWipAwUmqdskYmMK8wMV036ID`,
		{
			method: 'POST',
			body: form,
		}
...
function setToMisGifos(data) {
	return fetch(
		`https://api.giphy.com/v1/gifs/${data.data.id}?api_key=940D6rFjWipAwUmqdskYmMK8wMV036ID`
	)
		.then((response) => response.json())
...
function drawWaitingForVideo() {
	const title = (document.getElementById('title-screen').textContent =
		'¿Nos das acceso a tu cámara?');
	const p1 = (document.getElementById('p1-screen').textContent =
		'El acceso a tucámara será válido sólo');
	const p2 = (document.getElementById('p2-screen').textContent =
		'por el tiempo en que estés creando el GIFO');
	stepOneBtn.classList.add('active');
}
